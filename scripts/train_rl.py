...
# Custom imports
from baseline_models.DRRN.drrn_agent import DrrnAgent
from baseline_models.DRRN.drrn_trainer import DrrnTrainer
from baseline_models.offlineDRRN.offline_drrn_agent import offlineDrrnAgent
from baseline_models.offlineDRRN.offline_drrn_trainer import offlineDrrnTrainer
...
def main():
    ...
    train_env = HMTJerichoEnv(args.data_path + '/' + args.data_dir_name,
                              'train',
                              fully=fully,
    ...
    if args.model_name == defs.DRRN:
        ...
        envs = VecEnv(args.num_envs, train_env)
        if args.model == 'DRRN':
            agent = DrrnAgent(tb, log, args, envs, None)
            trainer = DrrnTrainer(tb, log, agent, envs, None, args)     # omit the eval_env
        elif args.model == 'offlineDRRN':
            agent = offlineDrrnAgent(tb, log, args, envs, None)
            trainer = offlineDrrnTrainer(tb, log, agent, envs, None, args)  # omit the eval_env
        else:
