...
class offlineDrrnAgent:
    def __init__(
        self,
        tb: logger.Logger,
        log: Callable[..., None],
        args: Dict[str, Union[str, int, float]],
        envs: List[HMTJerichoEnv],
        action_models
    ):
    ...
    def load(self, weight_file: str, memory_file: str, model_dir: str):
        try:
            self.memory = pickle.load(
                open(pjoin(model_dir, f"{memory_file}.pkl"), 'rb'))
            self.network.load_state_dict(
                torch.load(pjoin(model_dir, f"{weight_file}.pt")))
    ...
    def save(self, step: int, model_dir: str):
        try:
            pickle.dump(self.memory, open(pjoin(model_dir, 'memory_{}.pkl'.format(step)), 'wb'))
            torch.save(self.network.state_dict(), pjoin(model_dir, 'weights_{}.pt'.format(step)))
        except Exception as e:
